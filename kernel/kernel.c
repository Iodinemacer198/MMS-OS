...
static const uint8_t font5x7[][7] = {
    ...
    [' ']={0,0,0,0,0,0,0}, ['#']={0x0A,0x1F,0x0A,0x0A,0x1F,0x0A,0x0A}, ['$']={0x04,0x0F,0x14,0x0E,0x05,0x1E,0x04},
    ['%']={0x18,0x19,0x02,0x04,0x08,0x13,0x03}, ['&']={0x0C,0x12,0x14,0x08,0x15,0x12,0x0D}, ['@']={0x0E,0x11,0x17,0x15,0x17,0x10,0x0E},
    ['.']={0,0,0,0,0,0x0C,0x0C}, [':']={0,0x0C,0x0C,0,0x0C,0x0C,0}, [',']={0,0,0,0,0x0C,0x04,0x08}, ['\'']={0x04,0x04,0x08,0,0,0,0},
...
static void framebuffer_draw_half_cell(int cell_x, int cell_y, uint32_t fg, uint32_t bg, bool upper) {
    for (uint32_t y = 0; y < 16; y++) {
        uint32_t rgb = (upper ? y < 8 : y >= 8) ? fg : bg;
        for (uint32_t x = 0; x < 8; x++)
            framebuffer_put_pixel(cell_x * 8 + x, cell_y * 16 + y, rgb);
    }
}
...
static void framebuffer_draw_char(int cell_x, int cell_y, unsigned char c, uint8_t cell_color) {
    ...
    if (c == 0xDF) {
        framebuffer_draw_half_cell(cell_x, cell_y, fg, bg, true);
        return;
    }
    if (c == 0xDC) {
        framebuffer_draw_half_cell(cell_x, cell_y, fg, bg, false);
        return;
    }
